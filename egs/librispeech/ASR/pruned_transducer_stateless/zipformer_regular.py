...
class Zipformer2EncoderLayer(nn.Module):
    ...
    src_key_padding_mask:  the mask for padding, of shape (batch_size, seq_len); True means
        ...
        src = src + self.feed_forward1(self.norm_ff1(src))
        ...
        src = src + self.sequence_dropout(self.conv_module1(self.norm_conv1(src), chunk_size=chunk_size,
        ...
        src = src + self.sequence_dropout(self.feed_forward2(self.norm_ff2(src)),
                                          float(self.ff2_skip_rate))
        ...
        src = src + self.sequence_dropout(self.conv_module2(self.norm_conv2(src), chunk_size=chunk_size,
                                                            src_key_padding_mask=src_key_padding_mask),
                                          float(self.conv_skip_rate))

        # src = src + self.sequence_dropout(self.balancer_ff3(self.feed_forward3(src)),
        #                                   float(self.ff3_skip_rate))
        src = src + self.sequence_dropout(self.feed_forward3(self.norm_ff3(src)),
                                          float(self.ff3_skip_rate))
...
class ActivationDropoutAndLinear(torch.nn.Module):
   ...
   mask is smaller.
    ...
    def __init__(self,
                 ...
                 initial_scale: float = 1.0  # not used
                 ):
        super().__init__()
