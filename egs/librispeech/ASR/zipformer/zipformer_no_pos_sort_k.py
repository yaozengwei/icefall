...
class RelPositionMultiheadAttentionWeights(nn.Module):
    ...
    def streaming_forward(
        ...
        Returns:
           ...
           - left context indexes to keep, of shape (batch_size, past_topk).
        """
        x = self.in_proj(x)
        query_head_dim = self.query_head_dim
        # pos_head_dim = self.pos_head_dim
        ...
        mean_left_attn_weights = attn_weights[..., :left_context_len].mean(dim=(0, 2))
        # Both of shape (batch_size, past_topk)
        kept_left_attn_weights, indexes = torch.topk(
            mean_left_attn_weights, k=past_topk, dim=1
        )

        # Update cached left contexts
        # (past_topk, batch_size, key_dim)
        kept_cached_key = torch.gather(
            cached_key, dim=0, index=indexes.transpose(0, 1).unsqueeze(-1).expand(-1, -1, query_dim)
        )
    ...
    def streaming_forward(
        ...
        Args:
            ...
            cached_val: cached attention value tensor of left context,
              ...
              where left_context_len = past_topk + chunk_size.
            left_context_len: number of left context frames.
            indexes: left context indexes to keep, of shape (batch_size, past_topk)
        ...
        # Update cached left contexts
        # (past_topk, batch_size, val_dim)
        indexes = indexes.transpose(0, 1).unsqueeze(-1).expand(-1, -1, x.shape[-1])
        # (past_topk, batch_size, val_dim)
        kept_cached_val = torch.gather(cached_val, dim=0, index=indexes)
...
class NonlinAttention(nn.Module):
    ...
    def streaming_forward(
        ...
        indexes = indexes.unsqueeze(0).unsqueeze(-1).expand(num_heads, -1, -1, x.shape[-1])
        # (num_heads, batch_size, past_topk, head_dim)
