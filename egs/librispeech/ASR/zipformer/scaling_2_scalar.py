...
class ActivationDropoutAndLinearFunction(torch.autograd.Function):
    ...
    def forward(ctx,
        ...
        ctx.save_for_backward(x, weight, bias, scale1, scale2, dropout_mask)
    ...
    def backward(ctx, ans_grad: Tensor):
        ...
        (x, weight, bias, scale1, scale2, dropout_mask) = saved
        ...
        if dropout_mask is not None:
            y_deriv = y_deriv * dropout_mask

        if scale2 is not None:
            y_orig_deriv = y_deriv * scale2
            scale2_deriv = (y_deriv * y_orig).sum(dim=list(range(x.ndim - 1)))
        else:
            y_orig_deriv = y_deriv
            scale2_deriv = None

        x_deriv = y_orig_deriv * func_deriv

        if scale1 is not None:
            x_orig_deriv = x_deriv * scale1
            scale1_deriv = (x_deriv * x_orig).sum(dim=list(range(x.ndim - 1)))
        else:
            x_orig_deriv = x_deriv
            scale1_deriv = None

        return x_orig_deriv, weight_deriv, bias_deriv, scale1_deriv, scale2_deriv, None, None, None
...
class ActivationDropoutAndLinear(torch.nn.Module):
   ...
   mask is smaller.
    ...
    def __init__(self,
        ...
        if use_scale:
            self.scale1 = nn.Parameter(torch.ones(in_channels))
            self.scale2 = nn.Parameter(torch.ones(in_channels))
    ...
    def forward(self,
        ...
        if self.scale1 is None or self.scale2 is None:
            scale1 = None
            scale2 = None
        else:
            scale1 = limit_param_value(
                self.scale1,
                min=self.scale_min,
                max=self.scale_max,
        ...
        return ActivationDropoutAndLinearFunction.apply(
            x, self.weight, self.bias, scale1, scale2, self.activation,
...
class SharedWeightLinear(nn.Module):
    ...
    def __init__(
        self,
        linear: nn.Module,
        weight_scale_min: float = 0.1,
        weight_scale_max: float = 2.0,
...
class SharedWeightConv1d(nn.Module):
    """
    """
    def __init__(
        self,
        ...
        # than just setting it to None but I don't know what it is, maybe
        # something to do with exporting the module..
        self.register_parameter('bias', conv.bias)
        self.weight_scale = nn.Parameter(torch.ones(self.weight.shape[0], 1, 1))
        self.weight_scale_min = weight_scale_min
        self.weight_scale_max = weight_scale_max
        self.weight_scale_limit_prob = weight_scale_limit_prob
        ...
        for use_scale in [False, True]:
            ...
            for dropout_p in [0.0]:
                for activation in ['SwooshL', 'SwooshR']:
                    ...
                    if use_scale:
                        m1 = [Scale(in_channels, scale_limit_prob=1.0)] + m1[0:1] + [Scale(in_channels, scale_limit_prob=1.0)] + m1[1:]
                    m1 = nn.Sequential(*m1)
                    ...
                    if use_scale:
                        m1_linear = m1[4]
                        scale1 = torch.randn(out_channels, 1)
                        scale2 = torch.randn(out_channels, 1)
                        m1[0].scale.value = scale1
                        m1[2].scale.value = scale2
                        m2.scale1.value = scale1
                        m2.scale2.value = scale2
                    else:
                        m1_linear = m1[2]
                    ...
                    if use_scale:
                        ...
                        print("m2.scale1.grad = ", m2.scale1.grad)
                        assert isclose(m1[0].scale.grad, m2.scale1.grad)

                        print("m1[2].scale.grad = ", m1[2].scale.grad)
                        print("m2.scale2.grad = ", m2.scale2.grad)
                        assert torch.allclose(m1[2].scale.grad, m2.scale2.grad, atol=1.0e-05)
