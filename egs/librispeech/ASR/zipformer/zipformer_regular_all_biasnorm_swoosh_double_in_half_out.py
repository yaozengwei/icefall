...
class Zipformer2EncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        # self.norm_self_attn_weights = nn.LayerNorm(embed_dim)
        self.norm_self_attn_weights = BiasNorm(embed_dim)

        self.self_attn1 = SelfAttention(embed_dim, num_heads,
                                        value_head_dim)
        # self.norm_self_attn1 = nn.LayerNorm(embed_dim)
        self.norm_self_attn1 = BiasNorm(embed_dim)

        self.self_attn2 = SelfAttention(embed_dim, num_heads,
                                        value_head_dim)
        # self.norm_self_attn2 = nn.LayerNorm(embed_dim)
        self.norm_self_attn2 = BiasNorm(embed_dim)

        self.feed_forward1 = FeedforwardModule(embed_dim,
                                               (feedforward_dim * 3) // 4,
                                               dropout)
        # self.norm_feed_forward1 = nn.LayerNorm(embed_dim)
        self.norm_feed_forward1 = BiasNorm(embed_dim)

        self.feed_forward2 = FeedforwardModule(embed_dim,
                                               feedforward_dim,
                                               dropout)
        # self.norm_feed_forward2 = nn.LayerNorm(embed_dim)
        self.norm_feed_forward2 = BiasNorm(embed_dim)

        self.feed_forward3 = FeedforwardModule(embed_dim,
                                               (feedforward_dim * 5) // 4,
                                               dropout)
        # self.norm_feed_forward3 = nn.LayerNorm(embed_dim)
        self.norm_feed_forward3 = BiasNorm(embed_dim)

        self.nonlin_attention = NonlinAttention(embed_dim,
                                                hidden_channels=3 * embed_dim // 4)
        # self.norm_nonlin_attention = nn.LayerNorm(embed_dim)
        self.norm_nonlin_attention = BiasNorm(embed_dim)

        self.conv_module1 = ConvolutionModule(embed_dim,
                                              cnn_module_kernel,
                                              causal=causal)
        # self.norm_conv_module1 = nn.LayerNorm(embed_dim)
        self.norm_conv_module1 = BiasNorm(embed_dim)

        self.conv_module2 = ConvolutionModule(embed_dim,
                                              cnn_module_kernel,
                                              causal=causal)
        # self.norm_conv_module2 = nn.LayerNorm(embed_dim)
        self.norm_conv_module2 = BiasNorm(embed_dim)
    ...
    src_key_padding_mask:  the mask for padding, of shape (batch_size, seq_len); True means
        ...
        attn_weights = self.self_attn_weights(
            # src,
            self.norm_self_attn_weights(src),
            pos_emb=pos_emb,
            attn_mask=attn_mask,
        ...
        src = src + self.feed_forward1(self.norm_feed_forward1(src))
        ...
        na = self.nonlin_attention(self.norm_nonlin_attention(src), selected_attn_weights)

        src = src + (na if self_attn_dropout_mask is None else na * self_attn_dropout_mask)

        # self_attn = self.self_attn1(src, attn_weights)
        self_attn = self.self_attn1(self.norm_self_attn1(src), attn_weights)
        ...
        src = src + self.sequence_dropout(self.conv_module1(self.norm_conv_module1(src), chunk_size=chunk_size,
        ...
        #                                   ff2_skip_rate)
        # src = src + self.sequence_dropout(self.feed_forward2(src),
        src = src + self.sequence_dropout(self.feed_forward2(self.norm_feed_forward2(src)),
                                          ff2_skip_rate)
        ...
        self_attn = self.self_attn2(self.norm_self_attn2(src), attn_weights)
        ...
        src = src + self.sequence_dropout(self.conv_module2(self.norm_conv_module2(src), chunk_size=chunk_size,
                                                            src_key_padding_mask=src_key_padding_mask),
                                          conv_skip_rate)
        ...
        #                                   ff3_skip_rate)
        # src = src + self.sequence_dropout(self.feed_forward3(src),
        src = src + self.sequence_dropout(self.feed_forward3(self.norm_feed_forward3(src)),
                                          ff3_skip_rate)
...
class FeedforwardModule(nn.Module):
    ...
    def __init__(self,
        ...
        self.out_proj = nn.Sequential(SwooshL2(),
                                      Dropout3(dropout, shared_dim=0),
                                      ScaledLinear(feedforward_dim, embed_dim, bias=True,
                                                   initial_scale=0.1))
...
class ConvolutionModule(nn.Module):
    ...
    def __init__(
        ...
        # )
        self.out_proj = nn.Sequential(SwooshR2(),
                                      ScaledLinear(bottleneck_dim, channels, bias=True,
                                                   initial_scale=0.05))
...
class SwooshL2(torch.nn.Module):
    def __init__(self, ):
        super(SwooshL2, self).__init__()
        self.activation = SwooshL()

    def forward(self, x: Tensor) -> Tensor:
        return self.activation(x * 2.0) * 0.5


class SwooshR2(torch.nn.Module):
    def __init__(self, ):
        super(SwooshR2, self).__init__()
