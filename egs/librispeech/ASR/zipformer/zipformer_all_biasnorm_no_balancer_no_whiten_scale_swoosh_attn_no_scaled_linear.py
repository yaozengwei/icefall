...
class Zipformer2EncoderLayer(nn.Module):
    ...
    src_key_padding_mask:  the mask for padding, of shape (batch_size, seq_len); True means
        ...
        na = self.balancer_na(self.nonlin_attention(
            self.norm_nonlin_attention(src), selected_attn_weights
        ))
        ...
        src = src + self.sequence_dropout(
            self.conv_module1(self.norm_conv_module1(src), chunk_size=chunk_size,
        ...
        src = src + self.sequence_dropout(
            self.balancer_ff2(self.feed_forward2(self.norm_feed_forward2(src))),
            ff2_skip_rate)
        ...
        if torch.jit.is_scripting():
            ...
            conv_skip_rate = float(self.conv_skip_rate) if self.training else 0.0
        src = src + self.sequence_dropout(
            self.conv_module2(self.norm_conv_module2(src),
                              chunk_size=chunk_size,
                              src_key_padding_mask=src_key_padding_mask),
            conv_skip_rate)
        ...
        src = src + self.sequence_dropout(self.balancer_ff3(
            self.feed_forward3(self.norm_feed_forward3(src))),
            ff3_skip_rate)
...
class FeedforwardModule(nn.Module):
    ...
    def __init__(self,
        ...
        #                                            initial_scale=0.1)
        self.out_proj = nn.Sequential(ScaledSwooshL(),
...
class ConvolutionModule(nn.Module):
    ...
    def __init__(
        ...
        # )
        self.out_proj = nn.Sequential(ScaledSwooshR(),
...
class ScaledSwooshL(nn.Module):
    def __init__(self):
        super().__init__()
        self.swoosh_l = SwooshL()

    def forward(self, x):
        return 0.5 * self.swoosh_l(2.0 * x)


class ScaledSwooshR(nn.Module):
    def __init__(self):
        super().__init__()
        self.swoosh_r = SwooshR()

    def forward(self, x):
        return 0.5 * self.swoosh_r(2.0 * x)
