...
class BypassModule(nn.Module):
    ...
    def __init__(
        ...
        # self.bypass_scale = nn.Parameter(torch.full((embed_dim,), 0.5))
        self.embed_dim = embed_dim
        # self.bypass_scale = torch.full((embed_dim,), 0.5, requires_grad=False)
        self.skip_rate = copy.deepcopy(skip_rate)
        self.straight_through_rate = copy.deepcopy(straight_through_rate)
        self.scale_min = copy.deepcopy(scale_min)
        self.scale_max = copy.deepcopy(scale_max)

    def _get_bypass_scale(self, batch_size: int, device: torch.device):
        # returns bypass-scale of shape (num_channels,),
        # or (batch_size, num_channels,).  This is actually the
        # scale on the non-residual term, so 0 correponds to bypassing
        # this module.
        bypass_scale = torch.full((self.embed_dim,), 0.5, requires_grad=False).to(device)
    ...
    def forward(self,
        ...
        """
        bypass_scale = self._get_bypass_scale(src.shape[1], src.device)
