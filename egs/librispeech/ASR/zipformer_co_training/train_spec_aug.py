...
from model_spec_aug import AsrModel
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="Loss lambda for co-training.",
    )
...
def get_model(params: AttributeDict) -> nn.Module:
    assert params.use_ctc and not params.use_transducer, "Only support CTC yet."
...
def get_spec_augment(params: AttributeDict) -> SpecAugment:
    assert not params.enable_spec_aug
...
def compute_loss2(
    ...
    with torch.set_grad_enabled(is_training):
        ctc_loss_label, ctc_loss_cosub = model(
    ...
    info["loss_label"] = ctc_loss_label.detach().cpu().item()
    info["loss_cosub"] = ctc_loss_cosub.detach().cpu().item()
...
def compute_loss(
    ...
    with torch.set_grad_enabled(is_training):
        ctc_loss, cosub_ce_loss = model(
            x=feature, x_lens=feature_lens, y=y,
            use_spec_aug=is_training, supervision_segments=supervision_segments,
        ...
        # TODO: test warmup on loss_cosub
        loss = loss_lambda * ctc_loss + (1.0 - loss_lambda) * cosub_ce_loss
    ...
    info["cosub_ce_loss"] = cosub_ce_loss.detach().cpu().item()
