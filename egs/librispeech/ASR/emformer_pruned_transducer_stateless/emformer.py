    ...
    *Emformer: Efficient Memory Transformer Based Acoustic Model for Low Latency
    ...
    Args:
      ...
      aux_layers (Sequence[int]):
        List of auxiliary layer indexes.
      d_model (int):
        Input dimension.
      nhead (int):
        Number of attention heads in each emformer layer.
    ...
    def __init__(
        ...
        assert num_encoder_layers - 1 not in aux_layers
        self.aux_layers = set(aux_layers + [num_encoder_layers - 1])
...
class Emformer(EncoderInterface):
    def __init__(
        ...
        self.encoder = EmformerEncoder(
            chunk_length // 4,
            list(range(0, num_encoder_layers - 1, aux_layer_period)),
            d_model,
            nhead,
            dim_feedforward,
            num_encoder_layers,
...
class RandomCombine(torch.nn.Module):
    ...
    def forward(self, inputs: Sequence[torch.Tensor]) -> torch.Tensor:
        ...
        Returns:
          A Tensor of shape (*, num_channels).  In test mode this is just the
          final input.
        """
    ...
    def _get_random_mixed_weights(
        ...
        """
        Return a tensor of random one-hot weights,
        of shape (num_frames, self.num_inputs),
        ...
        Returns:
          A tensor of shape (num_frames, self.num_inputs), which elements in
          [0..1] that sum to one over the second axis, i.e. ans.sum(dim=1) is
          all ones.
        """
        logprobs = (
