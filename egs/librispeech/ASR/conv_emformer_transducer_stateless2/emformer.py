...
class EmformerAttention(nn.Module):
    ...
    def _forward_impl(
        ...
        if left_context_key is not None and left_context_val is not None:
            ...
            # for memory
            if M > 0:
                matrix_bd[:, R:, :M] = torch.nn.functional.avg_pool2d(
            ...
            # for right_context
            if R > 0:
                matrix_bd[:, R:, M : M + R] = matrix_bd_utterance[
                    :, :, tot_left_length + U :
                ]
            ...
            # for right_context
            if R > 0:
                matrix_bd[:, R:, M : M + R] = self._get_right_context_part(
                    matrix_bd_utterance
                )
            # for utterance
            matrix_bd[:, R:, M + R :] = matrix_bd_utterance[:, :, :U]
        ...
        if need_weights:
            ...
            probs_memory = attention_probs[:, R:, :M].sum(dim=2)
            probs_frames = attention_probs[:, R:, M:].sum(dim=2)
            return (
    ...
    def forward(
        ...
        It concatenates three blocks: hard-copied right contextss and
        full utterance, as a `big` block,
...
class EmformerEncoderLayer(nn.Module):
    ...
    def _apply_attention_module_forward(
        ...
        if self.use_memory:
            memory = self.memory_op(utterance.permute(1, 2, 0)).permute(
                2, 0, 1
            )[:-1]
        else:
        ...
        output_right_context_utterance, _, _ = self.attention(
            utterance=utterance,
            lengths=lengths,
    ...
    def infer(
        ...
        (
            src_att,
            output_state,
        ) = self._apply_attention_module_infer(src, R, lengths, pos_emb, state)
        src = src + self.dropout(src_att)
