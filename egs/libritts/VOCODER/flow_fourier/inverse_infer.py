...
def get_parser():
    ...
    parser.add_argument(
        ...
        help="Wav list file of the training data set.",
    )
...
def infer_audio(
    ...
    with torch.inference_mode():
        for batch_idx, (audios, audio_lens, file_names) in enumerate(dataloader):
            ...
            inv_noises, _ = model.inverse_infer(
                audio=audios,
...
def main():
    ...
    params.suffix = f"inv-noise-epoch-{params.epoch}-avg-{params.avg}"
    if params.use_averaged_model:
        params.suffix += "-use-avg-model"

    params.wav_dir_pred = f"{params.exp_dir}/{params.suffix}-step-{params.n_timesteps}"
