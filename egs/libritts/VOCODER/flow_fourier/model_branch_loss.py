...
class Vocoder(nn.Module):
    ...
    def process_model(
        ...
        if not self.higher_order:
            vt = output[:, 0]
            return vt, None, None, branch_outputs[:, :, 0]
        else:
            ...
            return vt, x_mid, x_dest, branch_outputs[:, :, 0]
    ...
    def forward(
        ...
        # else:
        #     drop_rate = 0.0
        vt, x_mid, x_dest, branch_outputs = self.process_model(
            x=xt,
            audio_lens=audio_lens,
            cond=cond,
            t=t,
        ...
        # shape of branch_outputs: (batch_size, num_branches, time)
