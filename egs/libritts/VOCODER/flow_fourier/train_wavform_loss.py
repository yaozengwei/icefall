...
from model_wavform_loss import Vocoder
...
def get_parser():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.99,
        help="Decay value for EMA model.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="number of powers of two for t schedules",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="additional random term added to t and dest_t in addition to powers of 1/2 determined by skipping_levels",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.25,
        help="fraction of batches for which we apply consistency loss.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.2,
        help="time constant that determines decay of close_loss.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="scale of main aux loss for far-away prediction",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.25,
        help="scale of ref_loss1 in loss function, make sure xt prediction matches ema model.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help="scale of ref_loss2 in loss function, make sure vt prediction matches ema model",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=2.0,
        help="scale on consistency loss",
    )
...
def add_model_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help="Whether to use higher-order prediction for fast inference.",
    )
...
def compute_loss(
    ...
    with torch.set_grad_enabled(is_training):
        losses = model(
            ...
            skipping_levels=params.skipping_levels,
            t_sigma=params.t_sigma,
            consistency_fraction=params.consistency_fraction,
            close_loss_decay=params.close_loss_decay,
        )
        if use_aux_loss:
            (main_loss, dest_loss, consistency_loss, ref_loss1, ref_loss2, close_loss) = losses
            loss = (
                main_loss
                + params.dest_loss_scale * dest_loss
                + params.consistency_loss_scale * consistency_loss
                + params.ref_loss_scale1 * ref_loss1
                + params.ref_loss_scale2 * ref_loss2
                + params.close_loss_scale * close_loss
            )
        else:
            main_loss, wavform_loss = losses
            loss = main_loss + params.wavform_loss_scale * wavform_loss
    ...
    loss_info["wavform_loss"] = wavform_loss.detach().item() * batch_size
    if use_aux_loss:
        loss_info["dest_loss"] = dest_loss.detach().item() * batch_size
        loss_info["consistency_loss"] = consistency_loss.detach().item() * batch_size
        loss_info["ref_loss1"] = ref_loss1.detach().item() * batch_size
        loss_info["ref_loss2"] = ref_loss2.detach().item() * batch_size
        loss_info["close_loss"] = close_loss.detach().item() * batch_size
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        if batch_idx % params.valid_interval == 0 and not params.print_diagnostics:
            ...
            if tb_writer is not None:
                ...
                for (step, pred_audio, gt_audio) in infer_samples:
                    tb_writer.add_audio(
                        f"train/valid_pred_audio_step_{step}",
                        pred_audio,
                        params.batch_idx_train,
                        params.sampling_rate,
                    )
                    tb_writer.add_audio(
                        f"train/valid_gt_audio_step_{step}",
                        gt_audio,
                        params.batch_idx_train,
                        params.sampling_rate,
                    )
...
def compute_validation_loss(
    ...
    world_size: int = 1,
    rank: int = 0,
) -> Tuple[MetricsTracker, Tuple[int, np.ndarray, np.ndarray]]:
        ...
        for batch_idx, batch in enumerate(valid_dl):
            ...
            if batch_idx == 0 and rank == 0:
                ...
                for n_timesteps in (1, 2, 4, 8):
                    ...
                    pred_audio = pred_audio[0, :audio_lens[0].item()].data.cpu().numpy()
                    gt_audio = audio[0, :audio_lens[0].item()].data.cpu().numpy()
                    returned_sample.append((n_timesteps, pred_audio, gt_audio))
