...
class FrechetSpeechDistance:
    ...
    def load_speech_files(self, wav_dir, wav_list_file, dtype="float32"):
        ...
        logging.info("[Frechet Speech Distance] Loading speech from {}...".format(wav_dir))
        with open(wav_list_file) as f:
            wav_list = f.read().splitlines()
        for fname in wav_list:
            res = pool.apply_async(
                _load_speech_task,
                args=(os.path.join(wav_dir, fname), self.sample_rate, self.channels, dtype),
            )
    ...
    def score(
        ...
        if real_embds_path is not None and os.path.exists(real_embds_path):
            ...
            speech_real = self.load_speech_files(real_path, wav_list_file, dtype=dtype)
        ...
        if eval_embds_path is not None and os.path.exists(eval_embds_path):
            ...
            speech_eval = self.load_speech_files(eval_path, wav_list_file, dtype=dtype)
        ...
        logging.info(f"shape of embds_real: {embds_real.shape}")
        logging.info(f"shape of embds_eval: {embds_eval.shape}")
...
if __name__ == "__main__":
    ...
    score = FSD.score(
        ...
        real_embds_path=args.real_embds_path,
        eval_embds_path=args.eval_embds_path,
    )
