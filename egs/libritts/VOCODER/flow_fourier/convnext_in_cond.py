...
class ConvNeXt(nn.Module):
    ...
    def __init__(
        ...
        self.in_proj = nn.Conv1d(in_channels + cond_channels, channels, 1, bias=False)
    ...
    def forward(
        ...
        for block in self.convnext_blocks:
            x = block(x, cond=None, time_embed=time_embed, length_mask=length_mask)
...
class WavEncoder(nn.Module):
    """ConvNeXt-based encoder for wavform."""
    def __init__(self, win_length: int = 64, channels: int = 256, num_layers: int = 2):
        super().__init__()
        self.win_length = win_length
        self.in_proj = nn.Conv1d(win_length, channels, 1, bias=False)
        self.convnext_blocks = nn.ModuleList(
            [
        ...
        self.act = nn.LeakyReLU()
        self.out_proj = nn.Conv1d(channels, win_length, 1, bias=False)
    ...
    def forward(
        ...
        num_frames = (time + win_length - 1) // win_length
        pad_len = num_frames * win_length - time

        x = nn.functional.pad(x, (0, pad_len))
        x = x.reshape(batch_size, num_frames, win_length).transpose(1, 2)
        # x shape: (batch_size, win_length, num_frames)

        x = self.in_proj(x)
        # x shape: (batch_size, channels, num_frames)

        x_lens = (audio_lens + win_length - 1) // win_length
        length_mask = make_pad_mask(x_lens).logical_not().unsqueeze(1)
        ...
        x = x.transpose(1, 2).flatten(1)  # (batch_size, num_frames * win_length)
        x = x[:, :time]
