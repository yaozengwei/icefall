...
from model_branch_loss import Vocoder
...
def compute_loss(
    ...
    with torch.set_grad_enabled(is_training):
        losses = model(
            ...
            branch_drop_rate=0,  # set to 0 since we use branch_losses
    ...
    for i in range(len(branch_losses)):
        loss_info[f"branch_loss_{i}"] = branch_losses[i].detach().item() * batch_size
    if use_aux_loss:
