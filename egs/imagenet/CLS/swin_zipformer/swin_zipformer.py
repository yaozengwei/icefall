...
class SwinZipformer(nn.Module):
    ...
    def __init__(
        ...
        super(SwinZipformer, self).__init__()
        ...
        for i in range(num_encoders):
            ...
            zipformer_encoder_layer = ZipformerEncoderLayer(
...
class SwinEncoderLayer(nn.Module):
    """
    It applies the ZipformerEncoderLayer on the windows or shifted wondows.
    """
    def __init__(
        self,
        zipformer_encoder_layer: nn.Module,
        ...
        self.zipformer_encoder_layer = zipformer_encoder_layer
    ...
    def forward(self, x: Tensor, pos_emb: Tensor):
        ...
        x_windows = self.zipformer_encoder_layer(
...
class SwinEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList([
            SwinEncoderLayer(
                copy.deepcopy(zipformer_encoder_layer),
        ...
        for i in range(num_layers):
            ...
            self.layers[i].zipformer_encoder_layer.bypass.skip_rate = ScheduledFloat(
...
def _test_swin_zipformer():
    model = SwinZipformer(num_classes=10, fused_window_process=False)
    model = model.cuda()
