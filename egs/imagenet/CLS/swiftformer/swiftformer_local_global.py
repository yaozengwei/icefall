...
class EfficientAdditiveAttnetion(nn.Module):
    ...
    def __init__(self, in_dims=512, token_dim=256, num_heads=2, window_size=7):
        super().__init__()
    ...
    def forward(self, x):
        ...
        assert H % self.window_size == 0, H
        assert W % self.window_size == 0, W
        ...
        A = query_weight * self.scale_factor  # (num_windows*B, window_size*window_size, 1)

        A = A.softmax(dim=1)

        G = torch.sum(A * query, dim=1)  # (num_windows*B, C)
...
class SwiftFormerEncoder(nn.Module):
    ...
    def __init__(self, dim, mlp_ratio=4.,
        ...
        self.pooling = PoolingModule(dim=dim)
    ...
    def forward(self, x):
        ...
        if self.use_layer_scale:
            ...
            x = x + self.drop_path(self.layer_scale_2 * self.pooling(x))
            x = x + self.drop_path(self.layer_scale_3 * self.linear(x))

        else:
            x = x + self.drop_path(
                self.attn(x.permute(0, 2, 3, 1)).permute(0, 3, 1, 2))
            x = x + self.drop_path(self.pooling(x))
...
@register_model
def SwiftFormer_XS_local_global(pretrained=False, **kwargs):
    model = SwiftFormer(
...
@register_model
def SwiftFormer_S_local_global(pretrained=False, **kwargs):
    model = SwiftFormer(
...
@register_model
def SwiftFormer_L1_local_global(pretrained=False, **kwargs):
    model = SwiftFormer(
...
@register_model
def SwiftFormer_L3_local_global(pretrained=False, **kwargs):
    model = SwiftFormer(
...
def test_model():
    ...
    model = SwiftFormer_XS_local_global(num_classes=1000)
    y = model(x)
